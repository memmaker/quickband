...
static void calc_hitpoints(void)
{
	...
	bonus = MIN(adj_con_mhp[p_ptr->state.stat_ind[A_CON]],20*p_ptr->lev+MAX(adj_con_mhp[p_ptr->state.stat_ind[A_CON]]-20*p_ptr->lev,0)/3);
...
static void calc_bonuses(void)
{
	...
	/* Affect Skill -- magic devices (INT). Note rebasing below  */
	p_ptr->state.skills[SKILL_DEVICE] += adj_int_dev[p_ptr->state.stat_ind[A_INT]];

	/* Affect Skill -- saving throw (WIS). Note rebasing below */
	...
	p_ptr->state.skills[SKILL_DEVICE] = MAX(0,p_ptr->state.skills[SKILL_DEVICE]-8); /* rebased adj_int_dev */

	/* Affect Skill -- saving throw (Level, by Class) */
	p_ptr->state.skills[SKILL_SAVE] += (cp_ptr->x_sav * p_ptr->lev / 10);

	p_ptr->state.skills[SKILL_SAVE] = MAX(0,p_ptr->state.skills[SKILL_SAVE]-8);   /* rebased adj_wis_sav */
	...
	{
 		int str, dex, str_index, dex_index;
 		int mult;
