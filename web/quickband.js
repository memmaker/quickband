/*
 * Quickband in the browser: terminal rendering, input and save persistence.
 * The game (quickband-core.js / .wasm) calls into Module.qb (see main-web.c).
 */
(function () {
	'use strict';
	...
	function makeWM() {
		...
		wm = RvipWM({
			...
			wins: [{ id: 'main', title: 'Map' }, { id: 'inv', title: 'Inventory' }, { id: 'msg', title: 'Messages' }, { id: 'mon', title: 'Visible monsters' }, { id: 'rec', title: 'Recall' }, { id: 'itm', title: 'Visible items' }, { id: 'eqp', title: 'Equipment' }],
	...
	function loadSoundCfg() {
		audio.cfg = {};
		var t = '';
		try { t = Module.FS.readFile('/quickband/lib/xtra/sound/sound.cfg', { encoding: 'utf8' }); } catch (e) { }
		t.split('\n').forEach(function (l) {
	...
	function exportSave() {
		...
		setTimeout(function () {
			...
			a.download = 'quickband-' + p.split('/').pop().replace(/^\d+\./, '') + '.sav';
	...
	window.addEventListener('error', function (e) {
		if (e.error instanceof WebAssembly.RuntimeError || /quickband-core/.test(e.filename || ''))
